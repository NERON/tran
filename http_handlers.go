	...
	for _, intervalStr := range intervals {
		...
		for e := bestSequenceList.Front(); e != nil; e = e.Next() {
			...
			if previousAddedSeq < sequenceData.Sequence-1 && sequenceData.Sequence > 2 && groupCount < 3 {
				...
				segmentsMap[fmt.Sprintf("%s_%d(%f)%s", intervalStr, sequenceData.Sequence-1, percentage, sign)] = SequenceResult{Interval: intervalStr, Val: sequenceData.Sequence - 1, Up: up, Down: down, Count: 1}
			}
