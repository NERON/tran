			...
			if _, ok := lowsMap[idx]; ok {
				...
				for _, cR := range centralRSIs {
					...
					if bestPeriod > 2 || (bestPeriod == 2 && candle.LowPrice <= up) {
						...
						if candle.OpenTime == 1621612800000 || candle.OpenTime == 1603809600000 || candle.OpenTime == 1602691200000 || candle.OpenTime == 1609300800000 || candle.OpenTime == 1572052800000 || candle.OpenTime == 1536460800000 {
